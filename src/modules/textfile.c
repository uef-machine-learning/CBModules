...
double *ReadRawVector(FILE *F, int *Dimensionality, int *Incomplete, int *Errors) {
  double *Larger;
  int k;
  char *Str, *Buffer;
  ...
  *Incomplete = *Errors = 0;
  if (!BufferVector)
    BufferVector = malloc(BufVectorSize * sizeof(double));
  if (*Dimensionality < 1)
    *Dimensionality = 1;
  do {
    ...
    while (*Str && isspace((int)*Str))
      Str++; /* move to first non-space */
    if (*Str == '%' || *Str == '#')
      *Str = 0;    /* jump over comments */
...
int GetRawFileInfo(char *Filename, int IgnoreIncomplete, int FrequencyLocation, int QuietLevel,
                   double *Minimum, double *Maximum, int *Dimensionality, int *Count,
                   int *HasIncomplete, int *HasErrors, int *FrequenciesOK, int *HasHeader,
                   int *TotalFrequency) {
  double *Vector;
  int MinMaxSet = 0;
  ...
  *HasHeader = 0;
  while (!feof(F)) {
    ...
    *HasIncomplete = *HasIncomplete || Incomplete;
    ...
    if (Incomplete && !IgnoreIncomplete)
      return 0;
    if (!Incomplete)
      for (k = 0; k < LatestDim; k++) {
        if (((k == 0) && (FrequencyLocation == 1)) ||
          ...
          if (((int)Vector[k]) - Vector[k]) {
            ...
            *TotalFrequency += (int)Vector[k];
        } else {
          if (!MinMaxSet) {
            *Minimum = *Maximum = Vector[k];
            MinMaxSet = 1;
          } else if (Vector[k] < *Minimum)
            *Minimum = Vector[k];
          else if (Vector[k] > *Maximum)
            *Maximum = Vector[k];
        }
      }
    else
...
void Data2CB(float **Data, CODEBOOK *CB, int count, int dim, float **MinMax, int bytes, int scaling,
             int ql) {
  int maxval, i, j;
