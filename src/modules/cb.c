...
llong NearestInCluster(TRAININGSET* TS, PARTITIONING* P, VECTORTYPE v,
                       int cluster, YESNO debug)
{
  int      i, nearest=0;
  ...
  if(debug)
    ...
    PrintMessage("Input: "); PrintVector(v, (TS)->BlockSizeX, (TS)->BlockSizeY);
    PrintMessage("\n");
    }
  ...
  while( i != ENDPARTITION )
     ...
     if( i<0 || i>=P->TSsize) ErrorMessage("ERROR: Looping outside %i \n", i);
     Dist = VectorDist(v, Vector(TS,i), VectorSize(TS));
     if (Dist < MinDist)
        {
        MinDist = Dist;
        nearest = i;
        if(debug) PrintMessage("Dist(%i)=%s  \n", i, FormattedValue(Dist));
        }
     i = NextVector(P, i);
     }
  if(debug) PrintMessage("\nNearest=%i \n", nearest);
                       ...
                       TRAININGSET* TS)
{
  ...
  if(booksize)  AllocateMemoryForBook(CB);
}
...
void AddToPartition(TRAININGSET* TS,
                           PARTITIONING* P,
                           int Pindex,
                           int item)
{
...
void WriteIntegerList(char* FileName,
                       int* list,
                       int* N)
{
    int   i;
...
void WriteDoubleList(char* FileName,
                       double* list,
                       int N)
{
    int   i;
...
void WritePartitioning2(char* FileName,
                       PARTITIONING* P,
                       TRAININGSET* TS,
                       int AllowOverWrite, int writeHeader)
{
  int   i;
  FILE *f;
                   ...
                   CODEBOOK*  CB2)
{
  ...
  if( NO )  // Debugging purpose. Turn on if needed
    ...
    printf("MSE Distortion = %9.4f %9.4f %9.4f\n",
           ...
           PrintableError(mse2,CB2),
           PrintableError((mse1+mse2)/2.0,CB1));
    }
...
char *FormattedValue(llong v)
{
  ...
  while (v>=1)
     {
     sprintf(tmp, s);
     if(v>=1000)  sprintf(s, ",%03d%s", (int) FloatModulo(v,1000), tmp);
     else         sprintf(s, "%d%s",    (int) FloatModulo(v,1000), tmp);
     v = (llong) v / 1000;
     }
...
int GraphRemoveEdge(Graph *g, int from , int to)
{
    ...
    for(int i=0;i<v->k;i++) {
        if (v->kindices[i]==to) {
            ...
            v->k--;
            if(i < v->old_k) {v->old_k--;} //TODO: verify
            break;
        }
    }
}
