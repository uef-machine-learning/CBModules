...
#define VersionNumber  "V.0.39"
#define LastUpdated    "16.06.2009" /* PF */

/* ----------------------------------------------------------------- */
/*      Changelog:                                                   */
/*                                                                   */
/* PF   0.39  BugFix in Remapping routine.                           */
                      ...
                      DISTANCETYPE    disttype)
{
  ...
  /* Part 1: Making updates for every partition */
  for( part = 0; part < BookSize(CB); part++ )
    {
    j = prev = FirstVector(P, part);

    /* Processing all vectors in partition part */
    while (! EndOfPartition(j))
      {
      /* Fix 16.6.09: Previously >= 2, but else clause missing (PF/IS) */
      if( UniqueVectors(P,part) >= 1 )
        {
        best = -1;
        ...
        /* Calculating distance to all cluster centroids */
        ...
        /* Vector remains in its current partition */
        if( best < 0 )
          {
          prev = j;
          j = NextVector(P, j);
          }
        /* Vector changes its partition */
        else
          {
        ...
        } /* else: vector has now changes its partition */

      /* Partition has only 1 vector */
      else
        {
        printf("Remapping ERROR: ONLY 1 VECTOR IN PARTITION: %i \n", UniqueVectors(P,part));
        }      
      }
    }

  /* Part 2: Evaluate those clusters that have changed */
  for( i = 0; i < BookSize(CB); i++ )
    {
    if( reevaluate[i] == YES )
      {
