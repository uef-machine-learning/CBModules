...
/*                Juha Kivij„rvi  (GenerateSearchTree)               */
/*                                                                   */
...
#define VersionNumber  "V.0.35a"
#define LastUpdated    "18.06.02" /* iak */
...
enum { CurrentAndSigma, CurrentPlusMinusSigma, CurrentAndFurthest, nv4, CurrentAndRandom, TwoFurthest, TwoRandom, MeansOfCurrentAndFurthest };
                                     ...
                                     int*          pivotindex)
{
  ...
  double   D1, D2, Dtot, Dmin = MAXDOUBLE;
   ...
   Returns non-zero if success. */
{
  ...
  else /* There is no different vectors in the partition. */
    {
    success = 0;
    }
                      ...
                      DISTANCETYPE    disttype)
{
  ...
  for( j = 0; j < BookSize(TS); j++ )
    {
    if( UniqueVectors(P,Map(P,j)) >= 2 )
      {
      best = -1;
      olddist = VectorDistance(Vector(CB, Map(P,j)),
                               Vector(TS, j),
                               VectorSize(TS),
                               MAXLLONG,
                               disttype);
      ...
      if( best > -1 )
        {
        if( reevaluate[Map(P,j)] == NO )
          {
          if( Value(PartitionErrorType) != SplitAllPartitions &&
              Map(P,j) != codevectorindices[0] &&
              Map(P,j) != codevectorindices[1] )
            {
            if( DeleteNodeFromBintree(Tree, &(CI[Map(P,j)]), NULL) == NULL )
              {
              printf("Del: j=%i map=%i best=%i\n", j, Map(P, j), best);
              printf("dist=%f\n", (double)CI[Map(P,j)].DistortionValue);
  /*             PrintBintree(Tree); */
              }
            }
          reevaluate[Map(P,j)] = YES;
          }

        ChangePartition(TS, P, best, j);
        }
      }
    }
