...
void PrintTime(double start)
{
  ...
  switch( Value(QuietLevel) )
    {
    case 0:  break;
    case 1:  printf("%6.1f ", elapsed);
             break;
    default: printf("Time: %6.1f\n", elapsed);
             break;
    }
}
...
void PrintTime(long start)
{
  ...
  switch( Value(QuietLevel) )
    {
    case 0:  break;
    case 1:  printf("%6li ", elapsed);
             break;
    default: printf("Time:%6li\n", elapsed);
             break;
    }
}
...
static void PrintOperatingInfo(char  TSetName[],
                               char  InitialCBName[],
                               char  CBName[])
{
  if( Value(QuietLevel) >= 2 )
    ...
    printf("Initial Codebook: %s\n", (InitialCBName[0] == 0x00
                                     ? "Centroid of the TRAINING SET"
                                     : InitialCBName) );
...
static void GenerateInitialCodebook(char*         InitialCBName,
                                    TRAININGSET*  TS,
                                    CODEBOOK*     CB,
                                    PARTITIONING* P)
{
                       ...
                       SASchedule*   SAS)
{
  ...
  SetSplitParameters(Value(QuietLevel),
                     Value(PartitionErrorType),
                     Value(NewVectors),
                     Value(NewVectorsHeuristically),
                     Value(Hyperplane),
                     Value(HyperplanePivot),
                     Value(HyperplaneLine),
                     Value(PartitionRemapping),
                     Value(LocalGLA),
                     Value(LocalGLAIterations),
                     Value(GLAIterations));
